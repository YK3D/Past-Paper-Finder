...
async function checkBanList(type, value) {
  ...
  const rows = await db('ban_list?type=eq.' + type + '&value=eq.' + encodeURIComponent(value) + '&select=reason&limit=1');
...
export default async function handler(req, res) {
  ...
  if (action === 'register') {
    ...
    const [ipBan, emailBan, userBan, devBan] = await Promise.all([
      ...
      deviceId ? checkBanList('device', deviceId) : Promise.resolve(null)
    ]);
    ...
    const result = await db('users', 'POST', {
      email: email.toLowerCase().trim(), username: username.trim(),
      password_hash: encoded, ip, device_id: deviceId || null
    });
  ...
  if (action === 'login') {
    ...
    await db('users?id=eq.' + user.id, 'PATCH', { ip, ...(deviceId ? { device_id: deviceId } : {}) });
